...
for k = 1:n_sys
    ...
    sigma = sim_systems(k).noise_baseline * sim_focus_noise_factor * sqrt(bandwidth);
    ...
    for ax = 1:lf.n_sensor_axes
        ...
        title(tl, sprintf('%s  |  source %.0f mm  |  sensor axis %d  |  %s array', ...
            sim_systems(k).label, sim_focus_src_mm, ax, sim_array), ...
            'FontSize', 13, 'FontWeight', 'bold');
        subtitle(tl, sprintf(['noise %gx baseline (%g %s)  ->  \\sigma = %.3g %s ' ...
                              'over %g Hz;  source %g nA\\cdotm @ %g Hz'], ...
            sim_focus_noise_factor, sim_systems(k).noise_baseline, ...
            sim_systems(k).noise_unit, sigma, unit_str, bandwidth, ...
            sim_dipole_nAm, sim_freq), ...
            'FontSize', 9, 'Color', [0.4 0.4 0.4]);

        fname = sprintf('noisy_topoplot_%s_src%03dmm_%gx_sensorax%d_%s', ...
            sim_systems(k).short, round(sim_focus_src_mm), ...
            sim_focus_noise_factor, ax, sim_array);
