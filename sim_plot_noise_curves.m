...
for k = 1:n_sys
    leg_labels{k} = sprintf('%s  (1x = %g %s)', ...
        sys_labels{k}, sys_baseline(k), sys_units{k});
end
...
subtitle(tl, sprintf(['evoked burst %g nA\\cdotm @ %g Hz; broadband white noise ' ...
                      'over %g Hz, averaged over %d trials'], ...
    sim_dipole_nAm, sim_freq, sim_fs/2, sim_n_trials), ...
